...
public class DALDBFactory {
	...
	public enum Mode {
		...
		Test("platform-auth-test"), Production("platform-auth");
	...
	public DALDBFactory(Mode mode) {

		createFactory(mode);

	}

	public DALDBFactory(String puname, String username, String password,
			String url, String driver) {

		l.log(Level.INFO, "Creating {0} with address {1} and with driver {2}.",
				new Object[] { DALDBFactory.class.getName(), url, driver });

		try {
			createFactory(puname, username, password, url, driver);
		} catch (Exception e) {
			l.log(Level.SEVERE, "Could not create a database connection: {0}",
					e.getMessage());
		}
	...
	public DALDBFactory(String mode) {
		...
		l.log(Level.INFO, "Creating {0} with mode {1}.", new Object[] {
				DALDBFactory.class.getName(), mode });
	...
	private void createFactory(Mode mode) {
	...
	private void createFactory(String puname, String user, String password,
			String url, String driver) {
	...
	public DAODBAuthentication getDAOAuthentication() {
		...
		return new DAODBAuthentication(factory);

	}
	
    public DAODBAccessKey getDAOAccessKey() {

        return new DAODBAccessKey(factory);
